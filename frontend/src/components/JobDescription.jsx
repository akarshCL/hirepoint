...
const JobDescription = () => {
    ...
    return (
        <div className='max-w-6xl p-4 mx-auto my-10 border-2 rounded-md shadow-lg max-md:mx-4'>
            <div className='flex items-center justify-between'>
                ...
                onClick={isApplied ? null : applyJobHandler}
                    disabled={isApplied}
                    className={`rounded-lg max-md:hidden block  ${isApplied ? 'bg-gray-600 cursor-not-allowed' : 'bg-[#7209b7] hover:bg-[#5f32ad]'}`}>
            ...
            <Button
                onClick={isApplied ? null : applyJobHandler}
                    disabled={isApplied}
                    className={`rounded-lg max-md:block hidden ${isApplied ? 'bg-gray-600 cursor-not-allowed' : 'bg-[#7209b7] hover:bg-[#5f32ad]'}`}>
                    {isApplied ? 'Already Applied' : 'Apply Now'}
                </Button>
        </div>
    )
